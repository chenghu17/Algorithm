...
// 基本思想是三个数字的从个位数开始相加，如果当前位数已经超过了字符串长度，则返回0与其他数字进行相加，
// 相加的结果考虑进位和当前位的数组，最后将结果进行字符串拼接，直到三个数每位都加完，则返回三个数的结果。
//
...
string addStrings(string num1, string num2, string num3) {
    ...
    string summary = "";
    for (int i = (int)num1.length(), j = (int)num2.length(), k = (int)num3.length(); i > 0 || j > 0 || k > 0 || carry > 0;) {
...
int main() {
    ...
    int stairs_num;
    cin >> stairs_num;
    string count = StepUp(stairs_num);
    cout << "the sum of methods: " << count;
    return 0;
}
